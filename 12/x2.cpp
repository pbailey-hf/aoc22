...
public:
    ...
    Map(std::istream& is)
    {
        for (std::string line; std::getline(is, line); ) {
            ...
            {
                if (c == 'S' || c == 'a') {
                    starts.emplace(storage_.back().size(), storage_.size()-1);
                    c = 'a';
                } else if (c == 'E') {
...
int main()
{
    ...
    path.starts = map.starts;
    path.goal = map.goal;
    Vec2 cur = map.goal;
    while (!map.starts.contains(cur)) {
